...
- **Fix: nested tool schemas flattened (issue #44)** — converting pi schemas to Zod for `createSdkMcpServer` collapsed nested objects to open records and dropped `anyOf`/`const`, so Claude only saw the top level of tools like `subagent`. The MCP server is now built directly and answers `tools/list` with pi's JSON Schema verbatim. Also drops SDK argument validation, which could skip a handler and desync the toolCallId cursor.
