...
- **Fix: AGENTS.md and system prompt** — Claude Code now correctly receives the same global and hierarchical `AGENTS.md` / `CLAUDE.md` files that Pi normally loads. You can set `provider.appendSystemPrompt` to `false` if you don't want these sent to CC.
