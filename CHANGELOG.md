...
- **Fix: Claude Code wrote auto-memory from bridge turns** — only the compact-summary subprocess set `CLAUDE_CODE_DISABLE_AUTO_MEMORY`, so provider and AskClaude turns could fork CC's extract-memories agent and append to `<claude config>/memory/MEMORY.md` and per-project topic files, as though a pi conversation had been an interactive Claude Code session. That agent runs its own tools, so `tools: []` did not stop it. All three subprocesses now share one env constant.
