...
- **Fix: session rebuilds destroyed the results of parallel tool calls** — pi records one message per tool result, Claude Code expects them in a single user message, and `repairToolPairing` (applied inside `Session.importMessages`) dropped every result past the first, substituting `[no tool result recorded]`. The conversion now collects a turn's results into one message, including when a mid-turn steer lands between them.
