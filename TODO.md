...
7. **Carry CC's `@file` attachments across a rebuild.** Claude Code expands an
   at-mention itself — pi expands prompt templates and skill commands but passes `@`
   through — and stores the expansion as a `type: "attachment"` record in its own
   session JSONL, holding the whole file under `attachment.content.file.content`. pi
   never sees it, so a rebuild writes a session without it and the model silently
   loses a file it was reasoning about. Observed: `req-0071` (resume) carries the
   source, `req-0073` (rebuild) has only the bare prompt text; it recovered two turns
   later only because the model happened to call `read` itself.

   Decided approach is to copy the records forward, not to re-expand from the prompt
   text — re-reading from disk can hand the model a file that changed since, and the
   different bytes break the cache anyway. Needs the cc-session-io work first (see
   that repo's TODO). The hard part is remapping `parentUuid`: an attachment points at
   the user record that mentioned the file, that record gets a new uuid on rebuild, and
   pi's text is not guaranteed byte-identical to CC's. Only content-bearing types
   matter (`file`, `edited_text_file`); `skill_listing`, `task_reminder`,
   `agent_listing_delta` and `mcp_instructions_delta` are regenerated by CC each turn.
   Exposure is small: 30 days holds 582 sessions with attachments, 34 bridge-created,
   10 content-bearing records among those.
