...
//
// Pi records one message per tool result; Claude Code puts them all in one user
// message. When the bridge wrote them pi's way, repairToolPairing — which runs
// inside Session.importMessages, so it cannot be opted out of — kept only the
// first and replaced the rest with a synthetic "[no tool result recorded]".
